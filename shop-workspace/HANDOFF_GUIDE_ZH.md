...
> `vision/HANDOFF_OCR_YOLO_ZH.md` 保留新增模块整合前的实验记录；其中“尚未串联”和 `.venv` 命令已过期。当前部署与运行以本文和根目录 `README.md` 为准。

本文给接手同学使用，目标是在一台新的 Windows 电脑上，从零配置 xArm 6、Orbbec Gemini 336 和 Robotiq 2F-85，并完成视觉识别、手眼标定与首次安全抓取。

## 1. 当前实现和交接状态

程序默认先移动到小票观察位，腕部 Gemini 336 拍摄桌面小票并通过 GLM-OCR 解析商品；只有严格匹配到一个已配置商品后，才移动到六种商品共用的抓取预备位。随后获取对齐的 RGB-D，YOLO 只检测小票指定类别，将检测框中心附近的深度从 2D 反投影为相机三维点，再通过眼在手上标定矩阵转换到基座坐标系；最后沿工具坐标系 Y 轴左右对齐、沿 X 轴上下对齐、沿 Z 轴深入，闭合夹爪，沿 X 轴轻抬，再沿 Z 轴退出。
...
正式入口已经整合 `vision/receipt_ocr.py` 的能力到 `xarm_grasp/receipt.py` 和 `xarm_grasp/__main__.py`。在 `config.json.receipt.observation` 中填写相机能清楚看到桌面小票的预定位姿，并在当前 PowerShell 设置 `$env:ZHIPUAI_API_KEY`。默认真实命令为 `python -m xarm_grasp --execute`：先看小票，再移动到共享抓取预备位抓取解析出的商品。`--item 商品名` 保留为绕过小票的调试入口，`--receipt-image 图片路径` 用于不重新拍小票的测试。

当前没有放置流程，因此只接受包含一个商品种类的小票。匹配不到商品或匹配到多个商品种类都会停止，不能自动选择其中一个。

## 2. 新电脑需要准备的硬件和系统

- Windows 10 或 Windows 11，64 位。
- Anaconda 或 Miniconda，64 位。环境名统一为 `xarm`，当前本机使用 Python 3.10.21。
- Git for Windows。
...
1. 整个 `shop-workspace` 源码，但不要复制本机 Conda 环境、`.test-deps`、`__pycache__` 和 `runs`。
...
不要复制原电脑的虚拟环境目录；在新电脑创建并激活同名 Conda 环境 `xarm`。

检查关键依赖：

```powershell
python -c "import cv2,numpy,ultralytics,pyorbbecsdk; print('Python dependencies OK')"
```
...
测试使用模拟机器人和合成数据，不会连接或移动机械臂。原抓取基线为 11 项测试；整合后新增 6 项小票解析测试，共 17 项。
...
```powershell
python ..\pyorbbecsdk\examples\quick_start.py
```
...
```powershell
python -c "from ultralytics import YOLO; print(YOLO('models/products.pt').names)"
```

## 8. 填写共享抓取预备位姿和商品配置

货架范围较小，六种商品共用顶层 `grasp_observation`，不再在每个商品中重复保存观察位姿。小票拍摄使用独立的 `receipt.observation`，因为它需要让腕部相机面向桌面小票。
...
在 Studio 中手动移动到抓取准备位置并停稳，然后读取位姿：
...
把输出写入顶层 `grasp_observation`。用相同方法示教看小票的位置，并写入 `receipt.observation`。

根据当前准备位置的工具坐标图，方向已确认为：Y+ 向左、Y- 向右；X- 向上、X+ 向下；Z+ 向前深入、Z- 退出。程序顺序为 Y± 左右对齐、X± 上下对齐、Z+ 深入、闭合、X- 向上轻抬、Z- 原路退出。`tool_motion` 配置为 `lateral_axis: "y"`、`vertical_axis: "x"`、`depth_axis: "z"`、`approach_axis_sign: 1`、`lift_axis_sign: -1`。

必须在 Studio 工具坐标点动模式下以很小步长验证正负方向。程序强制检查 X- 是否足够朝向基座上方；若准备姿态使 X- 不再向上就拒绝抓取。程序也会拒绝位于 Z- 方向的目标，避免反向深入。TCP 姿态在所有分轴动作中保持不变。

`grasp_depth_offset_mm` 表示从深度相机看到的瓶子正面表面，沿工具 Z 深入到 TCP 抓取中心还要移动的距离。该值仍按商品分别测量。还必须把现场验证的基座安全范围填入 `workspace_mm`。
## 9. 执行眼在手上标定
...
```powershell
python -m xarm_grasp.calibrate capture `
...
当前程序没有 `--plan-only` 或逐步确认模式。接手同学在首次真实抓取前，优先增加一种安全调试方式：到达观察位后只识别并保存 Y/X/Z 计划，不执行对齐和夹持；或让操作者对每一段动作单独确认。完成该功能后，再逐段验证 Y 左右、X 上下、Z 深入、X 抬起和 Z 退出。
...
5. **P0：示教观察位姿。** 填写一个共享 `grasp_observation` 和独立的小票观察位，并现场验证工具 Y/X/Z 方向。
6. **P0：测量安全参数。** 填写 `workspace_mm` 和每个商品的 `grasp_depth_offset_mm`，调整夹爪开闭位置。
7. **P0：增加计划预览或逐段确认。** 在第一次自动抓取前提供只计算、不抓取的运动计划调试入口。
8. **P0：逐段低速验收。** 共享抓取预备位、Y 对齐、X 对齐、Z 深入、夹持、X 抬起、Z 退出分别验证。
...
- [ ] 六种商品的模型类别、别名和抓取深度，以及共享抓取预备位已确认。
- [ ] 工具 Y=左右、X=上下、Z=深入/退出已经小步点动确认。
- [ ] 工作范围来自现场安全边界测量。
